...
function(a,b){var d=function(){c(this).dialog("close");c("#ui-error-rendered").remove();c("#ui-application-error").empty()},f={Close:d};if(a.type==="applicationNotice"){f={Cancel:d,Ok:function(){b.callback&&b.callback();d.call(this)}};if(!b.type)b.type="Confirm"}c("<div/>",{id:"ui-error-rendered",html:b.message}).appendTo("#ui-application-error");c("#ui-application-error").dialog({title:b.type,height:b.message?200:0,buttons:f,width:300,modal:true,autoOpen:true})});X=function(){return{typemap:{preview:"iframe",
embeddable:"textarea",full:"textarea"},exports:function(a){this.render[this.typemap[a.type]](a.parent,a.content)},render:{iframe:function(a,b){var d=c("<iframe/>",{id:"ui-preview-rendered"}).width("100%").height(a.height()-100);a.html(d);d=c("#ui-preview-rendered")[0];d=d.contentWindow?d.contentWindow:d.contentDocument.document?d.contentDocument.document:d.contentDocument;d.document.open();d.document.write(b);d.document.close()},textarea:function(a,b){var d=c("<textarea/>",{id:"ui-preview-rendered"}).width(a.width()-
100).height(a.height()-100);d.val(b);a.html(d)}}}}(y);i.bind("exportReady",function(a,b){var d=c("#ui-preview-viewer").dialog({modal:true,width:q.width()-50,height:q.height()-50,autoOpen:true,title:g(b.type).capitalize(),buttons:{Close:function(){c(this).dialog("close");c("#ui-preview-rendered").remove()}}});g.extend(b,{parent:d});X.exports(b)});O.delegate("li","click",function(a){z.addTrackEvent.call(this,a)});c("#ui-start-screen-list, #ui-user-videos").delegate("li","click",function(){var a=c(this),
b=a.data("track"),d=a.data("project");d&&J.project.load(b,d);a.parents("#ui-start-screen-list").length&&c("#ui-start-screen").dialog("close")});w.bind("resize",function(){e.trigger("timeupdate")});i.bind("addTrackComplete",function(){S.css({height:G.height()})});i.bind("removeTrackComplete",function(a,b){var d=h.getTrackEvents(e),f=b.type,j=0;g.each(d,function(k){k._id.indexOf(f)===0&&j++});if(!j){c('div[title="'+f+'"]').remove();delete F[f]}I.val()&&V.save()});i.bind("timelineReady videoReady",function(){S.css({height:G.height()})});
i.bind("videoLoadStart videoLoadComplete",function(a){a.type==="videoLoadStart"?T.show():T.hide()});i.bind("seekComplete",function(a,b){b.special&&b.special();b.type==="last"&&c("#ui-tracks").scrollLeft(c("#ui-tracks-time-canvas").width());b.type==="first"&&c("#ui-tracks").scrollLeft(0)});i.bind("videoEditComplete addTrackComplete",function(){var a=new TrackStore;ba.val(a.serialize(e.data.trackEvents.byStart))});var x=0,R=0,V={load:function(){R=x=0;var a=M.val(),b=/(.ogv)|(.mp4)|(.webm)/gi;!a||!b.test(a)?
i.trigger("applicationError",{type:!b.test(a)?"Invalid Movie Url":"No Video Loaded",message:"Please provide a valid movie url. ("+m.accepts.join(", ")+") "}):v.loadVideoFromUrl()},remove:function(){},save:function(){if(!e||!e.data)i.trigger("applicationError",{type:"No Video Loaded",message:"I cannot add a Track Event - there is no movie loaded."});else{var a=new TrackStore,b=I.val(),d=Q.val(),f=M.val(),j=D.attr("data-theme"),k=E.attr("data-layout"),l;if(b){l=g(b).slug();a.Title(b);a.Description(d);
a.Remote(f);a.Theme(j);a.Layout(k);a.read(l)?a.update(l,e.data.trackEvents.byStart):a.create(e.data.trackEvents.byStart);J.menu.load("#ui-user-videos");c("#ui-user-videos li[data-slug='"+l+"']").trigger("click")}else i.trigger("applicationError",{type:"No Title",message:"You will need to add a title in order to save your project."})}},play:function(){e.video.play()},pause:function(){e.video.pause()},volume:function(a){if(a==="up")R=e.video.volume+0.1;if(a==="down")R=e.video.volume-0.1;e.video.volume=
R},seek:function(a){if(a.indexOf(":")>-1){var b=c("#"+(a.split(":")[1]||""));x=g(b.val()).smpteToSeconds()}if(a==="first"){x=0;i.trigger("seeked","first")}if(a==="prev"){x=g(e.video.currentTime-0.25).fourth();i.trigger("seeked","prev")}if(a==="next"){x=g(e.video.currentTime+0.25).fourth();i.trigger("seeked","next")}if(a==="last"){x=e.video.duration;i.trigger("seeked","last")}if(x>e.video.duration)x=e.video.duration;if(x<0)x=0;e.video.currentTime=x;var d=setInterval(function(){if(e.video.readyState>=
3){i.trigger("seekComplete",{type:a,time:x});clearInterval(d)}},1)}};P.bind("click",function(a){if(e.video){c(this);var b=Math.round(c("#ui-tracks-time-canvas").width()/e.video.duration);a=g(a.offsetX/b).fourth();e.video.currentTime=a}});Y.bind("click",function(a){a.preventDefault();a=c(this);a.attr("data-control")&&V[a.attr("data-control")]()});Z.children("button").bind("click",function(){var a=c(this).children("span").children("span");!e||!e.data?i.trigger("applicationError",{type:"No Video Loaded",
message:"I cannot "+a.attr("data-control")+" - there is no video loaded"}):V[a.attr("data-control")](a.attr("data-opt"))});W.bind("keydown",function(a){a.which===13&&c("#io-current-time").next().trigger("click");a.which===39&&c('[data-opt="next"]').parents("button").trigger("click");a.which===37&&c('[data-opt="prev"]').parents("button").trigger("click")});y.$popcorn=e})})(this,this.document,this.jQuery,this._,this.Popcorn);