...
( function() {
  ...
  if ( has( 'source-config' )) {
    ...
    document.write( '<script data-butter-exclude="true">' + '( function() {' + 'var ctx = require.config({ ' + 'baseUrl: "' + path + '",' + 'context: "butter",' + 'paths: {' + 'external: "' + path + '../external",' + 'butter: "' + path + '"' +
    // Paths are relative to baseUrl; Notice the commas!
    '}' + '});' + 'ctx( [ "butter-main" ] )' + '})()' + '</script>' );
  }
