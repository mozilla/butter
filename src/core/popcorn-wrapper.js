...
define( [ "core/logger", "core/eventmanager" ], function( Logger, EventManager ) {
  ...
  return function( mediaId, options ) {
    ...
    var _id = mediaId,
      ...
      _onPrepare = options.prepare ||
    function() {}, _onFail = options.fail ||
    function() {}, _onPlayerTypeRequired = options.playerTypeRequired ||
    function() {}, _onTimeout = options.timeout ||
    function() {}, _url = options.setup && options.setup.url, _target = options.target && options.setup.target, _popcorn, _mediaType, _popcornTarget, _butterEventMap = {}, _mediaLoadAttempts = 0, _interruptLoad = false, _playerReady = false, _this = this;
    ...
    function constructPlayer( target ) {
      var script, targetElement = document.getElementById( target );
