...
define( [ "core/logger", "util/dragndrop", "./ghost-manager" ],
  ...
  return function( butter, media, mediaInstanceRootElement ) {
    ...
    _droppable = DragNDrop.droppable( _element, {
      ...
      drop: function( dropped, mousePosition, node ) {
    ...
    function createTrackEventFromDrop( trackEvent, popcornOptions, oldTrack, desiredTrack, node ) {
      var newTrack = _media.forceEmptyTrackSpaceAtTime( desiredTrack, popcornOptions.start, popcornOptions.end, trackEvent, node );

      if ( oldTrack !== newTrack ) {
        if ( oldTrack ) {
          oldTrack.removeTrackEvent( trackEvent, node );
        }
        trackEvent.update( popcornOptions, false, node );
